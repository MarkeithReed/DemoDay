 ...
 * http://sizzlejs.com/
 *
 * Copyright 2005, 2013 jQuery Foundation, Inc. and other 
...
// Can't do this because several apps 

including ASP.NET trace
...
xmlNode.method` instead of `xmlNode.method !== undefined`
...
jQuery.camelCase as callback to replace()
	...
	// The ready event handler and self 
...
equivalent to: $(context).find(expr)
			} else {
...
function the jQuery prototype for later instantiation
...
we're merging plain objects or arrays
...
window.$ === jQuery ) {
	...
	// A counter to track how many items to wait 
...
[[Class]] property is not "[object Object]"
		// - DOM nodes
		// - window
...
unique handler to the same of original handler, so it can be removed
...
calculations.
	...
	// If support gets modularized, this method should be moved 

back to the css module.
	swap: function( elem, options, callback, args ) {
		var ret, name,
			old = {};

		// Remember the old values, and 
...
arr.push,
	...
	booleans = "checked|selected|async|autofocus|autoplay|controls|defer|disabled|hidden|ismap|loop|multiple|open|
...
should be a CSS identifier http://www.w3.org/TR/css3-selectors/#attribute-selectors
...
the same handler for all of the specified attrs
...
elements found at the specified indexes
			while ( i-- ) {
...
/**
 * Detect xml
 * @param {Element|Object} elem An element or a 
...
node.ownerDocument || node : preferredDoc,
...
will throw "permission denied" error when accessing "document" variable, see jQuery #13936
...
attributes and not properties (excepting IE8 booleans)
...
node = typeof elem.getAttributeNode !== strundefined && elem.getAttributeNode("id");
...
context.getElementsByTagName !== strundefined ) {
...
test IE's treatment of not explicitly
...
does not throw on post-comma invalid pseudos
...
(support.matchesSelector = rnative.test( (matches = docElem.webkitMatchesSelector ||
...
in, an element does not contain itself
...
sortInput, a ) - indexOf.call( sortInput, b ) ) :
					0;
			}
...
expr.replace( rattributeQuotes, "='$1']" );
...
match[3] whether quoted or unquoted
...
elem.parentNode) && elem.nodeType === 1 );
				return false;
			};
		}),
...
this property makes selected-by-default
...
elem.parentNode.selectedIndex;
		...
		"empty": function( elem 
			...
			// :empty is only affected by element nodes and content nodes(including text(3), cdata
...
createPositionalPseudo(function() {
			return [ 0 ];
		}),
...
from seed or context
			elems = seed || multipleContexts( selector || "*", context.nodeType ? [ context ] : context, [] ),

			// Prefilter to 

get matcher input, preserving a map for seed-results synchronization
...
postFinder, or filtered seed, or non-seed postFilter or preexisting results,
...
elements from seed to results to keep them synchronized
				i = matcherOut.length;
				while ( i-- ) {
...
results, through postFinder if defined
		} else {
			matcherOut = condense(
				matcherOut === results ?
...
a shortcut and set the context if the root selector is an ID
...
assume duplicates if they aren't passed to the comparison function
...
return 1, but returns 4 (following)
	return div1.compareDocumentPosition( document.createElement("div") ) & 1;
});

// Support: IE<8
// Prevent attribute/property 
...
formatted options into Object-formatted ones and store in cache
 ...
 * Possible options:
 *
 *	once:			will ensure the 

callback list can only be fired once (like a Deferred)
 *
...
fire (used internally by add and fireWith)
		firingStart,
		// End of the loop when firing
		firingLength,
		// Index of currently firing 

callback (modified by remove if needed)
		firingIndex,
...
tuple[0] + "With" ]( this === deferred ? promise : this, arguments );
				return this;
			};
			deferred[ tuple[0] + 
...
i ] = arguments.length > 1 ? core_slice.call( arguments ) : value;
...
document.createElement("input");
	...
	// #11217 - WebKit loses check when the 

name is after the checked attribute
...
window.getComputedStyle because jsdom on node.js will break without it.
...
for non-element nodes in modern browsers,
...
Data.accepts( owner ) ) {
		...
		var descriptor = {},
			// Check if the owner object already has a 
...
for this "owner", create one inline
...
if ( jQuery.isEmptyObject( cache ) ) {
			...
			// Otherwise, copy the properties one-by-one to 
...
the cache object
	...
	get: function( owner, key ) {
		// Either a valid cache is found, or 
...
valid owner object must be provided.
...
return stored !== undefined ?
		...
		// [*]When the key is not a string, or 
...
converted to camelCase.
				...
				// both plain key and camelCase key. 
...
key with the spaces exists, use it.
...
data_user.access( elem, name, data );
	},
	...
	// TODO: Now that all calls to _data 

and _removeData have been replaced
...
elem.nodeType === 1 && !data_priv.get( elem, "hasDataAttrs" ) ) {
...
jQuery object (element matches) is not empty
			// (and therefore has an element appears at this[ 0 ]) and the
			// `value` parameter was not 

undefined. An empty jQuery object
			// will result in `undefined` for elem = this[ 0 ] which will
			// throw an exception if an attempt to 

read a data cache is made.
...
dataAttr( elem, camelKey, undefined );
				if ( data !== undefined ) {
					return data;
				}
...
we need to also store a copy of that
				// unchanged property.
...
null/undefined as ""; convert numbers to string
			if ( val == null ) {
				val = "";
			} else if ( typeof val === 
...
for an option in an optgroup can be inaccurate
...
managing events -- not part of the public interface.
...
namespace-only handlers
			...
			// If event changes its type, use the 

special event handlers for the changed type
...
api type, otherwise given type
			...
			// Update special based on newly reset 
...
the event handler queue if we're the first
...
|| [];
			...
			// Unbind all events (on this 

namespace, if provided) for the element
			if ( !type ) {
				for ( type in events ) {
...
document, then to window; watch for a global ownerDocument var (#9724)
...
=== false ) {
		...
		// If nobody prevented the 

default action, do it now
...
jQuery.isFunction( elem[ type ] ) && !jQuery.isWindow( elem ) ) {
...
the preDispatch hook for the mapped type, and let it bail if desired
...
they may want to stop propagation beneath us
		i = 0;
...
some event props shared by KeyEvent and MouseEvent
	props: "altKey bubbles cancelable ctrlKey currentTarget eventPhase metaKey relatedTarget shiftKey target timeStamp view 
...
original.keyCode;
	...
	mouseHooks: {
		props: "button buttons clientX clientY offsetX 
...
the document may have been marked as prevented
...
src;
	...
	// Create a timestamp if 

incoming event doesn't have one
...
mouse left/entered the browser window
...
jQuery.support.focusinBubbles ) {
	jQuery.each({ focus: "focusin", blur: "focusout" }, function( orig, fix ) {
		...
		// Attach a single capturing handler while 
...
event.target, jQuery.event.fix( event ), true );
			};
...
jQuery.dir( elem, "previousSibling", until );
	},
...
var rxhtmlTag = /<(?!area|br|col|embed|hr|img|
...
jQuery.filter( selector, this ) : this,
			i = 0;
...
keepData && jQuery.contains( elem.ownerDocument, elem ) ) {
...
element nodes and prevent memory leaks
						if ( elem.nodeType === 1 ) {
...
relevant into its fragment
		...
		this.domManip( arguments, function( 
...
the snapshot next if it has moved (#13810)
...
first, scripts, hasScripts, node, doc,
			i = 0,
			l = this.length,
			set = this,
			iNoClone = l - 1,
...
because core_push.apply(_, arraylike) throws
...
jQuery.support.noCloneChecked && ( elem.nodeType === 1 || elem.nodeType === 11 ) && !jQuery.isXMLDoc( elem ) ) {

			// We eschew Sizzle here for 

performance reasons: http://jsperf.com/getall-vs-sizzle/2
...
scripts as having already been evaluated
...
hidden ? display : jQuery.css(elem, "display") );
	...
	// Set the display of most of the 

elements in a second loop
...
property hooks for overriding the default
...
version
	...
	if ( value !== undefined ) {
		...
		if ( type === "string" && (ret = rrelNum.exec( value )) ) {
			...
			// Make sure that NaN and null 

values aren't set. See: #7116
...
value, otherwise just set the specified value
...
origName ] || ( jQuery.cssProps[ origName ] = vendorPropName( elem.style, origName ) );

		// gets hook for the prefixed version
		// followed by the unprefixed 
...
val === "normal" && name in cssNormalTransform ) {
			val = cssNormalTransform[ name ];
		}

		// Return, converting to number if 

forced or a qualifier was provided and val looks numeric
		if ( extra === "" || extra ) {
			num = parseFloat( val );
			return 

extra === true || jQuery.isNumeric( num ) ? num || 0 : val;
		}
		return val;
	}
});
...
this point, extra isn't content nor padding, so add border
			if ( extra !== "padding" ) {
...
val = curCSS( elem, name, styles );
		...
		// Computed unit is not 

pixels. Stop here and return.
		if ( rnumnonpx.test(val) ) {
			return val;
		}

		// we need the check for style in case a 

browser which returns unreliable values
...
the default display value of an element
...

{
		...
		// Use the already-created iframe if possible
			iframe = ( iframe ||
				jQuery("<iframe frameborder='0' width='0' 
...
a new HTML skeleton so Webkit and Firefox don't choke on reuse
...
getComputedStyle returns wrong value for margin-right
	...
	// getComputedStyle returns percent when 

specified for top/left/bottom/right
...
.is(":disabled") so that fieldset[disabled] works
...
jQuery.isFunction( value ) ? value() : ( value == null ? "" : value );
...
keydown keypress keyup error contextmenu").split(" "), function( i, name ) {
...
(see ajax/jsonp.js for an example)
	 * 2) These are called:
...
start with transport dataType and THEN go to "*" if needed
	 */
	transports = {},
...
function addToPrefiltersOrTransports( structure ) {
	...
	// dataTypeExpression is optional and 
...
"flat" options (not to be deep extended)
// Fixes #9887
...
of functions for handling common AJAX events
...
global events is callbackContext if it is a DOM node or jQuery collection
...
aborted inside a prefilter, stop there
		...
		// We can fire global events as of now if asked 
...
dataTypes[ 0 ] === "*" ) {
		dataTypes.shift();
		if ( ct === undefined ) {
			ct = s.mimeType || jqXHR.getResponseHeader("Content-
...
function (fires after converters)
...
jQuery.support.cors || xhrSupported && !options.crossDomain ) {
		return {
...
domain requests, seeing as conditions for a preflight are
...
jQuery.extend( true, { specialEasing: {} }, options ),
...
makes sure that the complete handler will be called
...
( hidden ? "hide" : "show" ) ) {
				...
				// If there is dataShow left over from a stopped hide or show and we are going to proceed with show, we should 
...
string as a 3rd parameter to .css will automatically
...
for any animations in the old queue and finish them
			for ( index = 0; index < length; index++ ) {
...
we don't have gBCR, just use 0,0 rather than error
...
position if either top or left is auto and position is either absolute or fixed
...
of 5/8/2012 this will yield incorrect results for Mobile Safari, but there
...
width or height on the element, requesting but not forcing parseFloat
...
// The number of elements contained in the matched 
...
// })();
if ( typeof module === "object" && module && typeof 
...
anonymous AMD modules. A named AMD is safest and most robust
	// way to register. Lowercase jquery is used because AMD module names are
	// derived from file names, and 

jQuery is normally delivered in a lowercase
	// file name. Do this after creating the global so that if an AMD module wants
	// to call noConflict to hide this version of 
...
object, that at least has a document property,
