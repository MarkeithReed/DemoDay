...
$.fn.alert = function (option) {
    return this.each(function () {
      ...
      if (!data) $this.data('alert', (data = new 
...
$(target).data('collapse') ? 'toggle' : $this.data()
...
backdrop"/>').insertBefore($(this)).on('click', clearMenus)
        }
...
(!isActive || (isActive && e.keyCode == 27)) {
      ...
      $items = $('[role=menu] li:not

(.divider):visible a', $parent)
...
&& /#/.test(selector) && selector.replace(/.*(?=#[^\s]*$)/, '') //strip for ie7
    }

    $parent = selector && $(selector)

    if (!$parent || !$parent.length) $parent = 
...
('click.dropdown.data-api', clearMenus)
...
this.$element = $(element)
    ...
    this.options.remote && this.$element.find('.modal-

body').load(this.options.remote)
  }
...
'toggle' : $.extend({ remote:!/#/.test(href) && href }, $target.data(), $this.data())
...
$element.on(eventIn + '.' + this.type, this.options.selector, $.proxy(this.enter, this))
...
().$element.off('.' + this.type).removeData(this.type)
    }
...
o.content == 'function' ? o.content.call($e[0]) :  o.content)
        || $e.attr('data-content')
...
'right'
  , trigger: 'click'
  , content: ''
  , template: '<div class="popover"><div class="arrow"></div><h3 class="popover-title"></h3><div class="popover-
...
!function ($) {
 ...
 var Affix = function (element, options) {
    ...
    this.$window = $(window)
      .on('scroll.affix.data-api', $.proxy
...
(reset).addClass('affix' + (affix ? '-' + affix : ''))
  }
